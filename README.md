...
**Note on Cell Parameter Package:** The registered PyBaMM cell parameter package developed in this study for NFPP/Hard-Carbon chemistry (`nfpp_sodium_ion`) is to be published and released as an open-source library for high-fidelity electrochemical simulations.

### 1. DFN-Based NFPP Cell Optimization & Benchmarking
A hierarchical multi-stage framework for cell design enhancement and performance benchmarking:
...
*   **Parameter Optimization**: Hierarchical search for structural ($\theta_s$) and material ($\theta_m$) parameters using sensitivity-based Jacobian screening and Genetic Algorithms (GA) to maximize energy capacity, continuous power capability, and cycle life.
*   **Thermo-Mechanical Envelope**: PDE-based structural strain check via a thermoelastic continuum model in FEniCSx to prevent degradation and damage initiation.
...
*   **ATP-EMTP Transient Coupling**: Real-time coupling of sub-cycle transients (such as transformer inrush, capacitor switching, motor starting, temporary faults, and non-linear switching noise) to extract high-frequency spectral and waveform features.
...
- `nfpp_sodium_ion/`: Registered, ready-to-be-published PyBaMM parameter set for NFPP/Hard-Carbon chemistry.
