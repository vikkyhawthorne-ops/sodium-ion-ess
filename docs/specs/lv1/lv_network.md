# 2. LV Network Parameters - LV Feeder 1
...
| Number of buses | $N_b$ | 20 |
| Number of branches | $N_l$ | 19 |
...
| Network base power | $S_{\mathrm{base}}$ | 1.5 MVA |
| Network base voltage | $V_{\mathrm{base}}$ | 415 V |

## 2.2 LV topology parameters

For each bus in LV Network 1:

| Bus ID ($b_i$) | Phase Availability ($A_i$) | Bus Type ($T_{\mathrm{bus}}$) | Parent Bus ($b_p$) | Grounding Connection ($G_{\mathrm{conn}}$) |
| --- | --- | --- | --- | --- |
| `feeder1_sec` | $\{A,B,C,N\}$ | Root / LV Secondary | `feeder1_head` | Solidly Grounded |
| `f1_node1` | $\{A,B,C,N\}$ | Intermediate | `feeder1_sec` | Grounded |
| `f1_node2` | $\{A,B,C,N\}$ | Intermediate | `feeder1_sec` | Grounded |
| `f1_node3` ... `f1_node19` | $\{A,B,C,N\}$ | Consumer Load Node | Known Tree Parent | Grounded |
