...
def define_downstream_network(scenario_idx: int, topology: str, f1_load_kw: float, f2_load_kw: float, f3_load_kw: float, line_len_mult: float):
    ...
    dss("new linecode.downstream nphases=3 r1=0.15 x1=0.45 r0=0.35 x0=1.45 c1=8.0 c0=4.0 units=km")
    ...
    dss(f"new line.down_f1 bus1=feeder1_sec bus2=f1_loadbus phases=3 linecode=downstream length={len1} units=km")
    dss(f"new line.down_f2 bus1=feeder2_sec bus2=f2_loadbus phases=3 linecode=downstream length={len2} units=km")
    dss(f"new line.down_f3 bus1=feeder3_sec bus2=f3_loadbus phases=3 linecode=downstream length={len3} units=km")

    # 2. Topology Reconfiguration: Radial vs Ring/Loop
    # If topology is 'ring', we close a tie-line between f1_loadbus and f2_loadbus
    if topology == 'ring':
        dss("new line.tieline bus1=f1_loadbus bus2=f2_loadbus phases=3 linecode=downstream length=0.4 units=km")

    # 3. Load Modeling: Linear and Non-Linear
    # Feeder 1 has varying linear loads
    dss(f"new load.f1_load bus1=f1_loadbus phases=3 kv=0.24 kw={f1_load_kw} pf=0.95 model=1 status=fixed")
    ...
    dss(f"new load.f2_load bus1=f2_loadbus phases=3 kv=0.24 kw={f2_load_kw} pf=0.82 model=2 status=fixed")

    # Feeder 3 has large varying industrial loads (linear/non-linear combo)
    dss(f"new load.f3_load bus1=f3_loadbus phases=3 kv=0.24 kw={f3_load_kw} pf=0.88 model=1 status=fixed")
